class NativeBeaconPersistence {
    ...
    static func setCallbackDispatcherHandle(_ handle: Int64) {
        persistentState.set(
            ...
            forKey: Constants.BEACON_CALLBACK_DISPATCHER_KEY
        )
    }
    
    static func getCallbackDispatcherHandle() -> Int64? {
        guard let handle = persistentState.value(forKey: Constants.BEACON_CALLBACK_DISPATCHER_KEY) else { return nil }
    ...
    private static func getRegionCallbackMapping() -> [AnyHashable: Any] {
        var callbackDict = persistentState.dictionary(forKey: Constants.BEACON_CALLBACK_DICT_KEY)
        if callbackDict == nil {
            callbackDict = [:]
            persistentState.set(callbackDict, forKey: Constants.BEACON_CALLBACK_DICT_KEY)
        }
        return callbackDict!
    }
    
    private static func setRegionCallbackMapping(_ mapping: inout [AnyHashable: Any]) {
        persistentState.set(mapping, forKey: Constants.BEACON_CALLBACK_DICT_KEY)
    }
}
